/**
 * SCORE taxonomy config (Jelson Type/Subtype scheme). Since the A/B classifier
 * was removed, this is kept only as the suggestion source for the New Intent
 * modal: 4 fixed Types, each with Subtypes carrying a code, label, and
 * description. Client-safe (no server/openai imports).
 */
...
/** Cap seed example queries so an import/edit can't blow up the config. */
...
export function normalizeConfig(raw: unknown): ScoreConfig | null {
  ...
  for (const t of typesRaw) {
    ...
    for (const s of subtypesRaw) {
      ...
      // Codes are normalized to uppercase for stable lookups everywhere.
      ...
      subtypes.push({ code, label, description });
    }
    types.push({
      key,
