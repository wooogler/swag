...
/**
 * Word-level diff of two texts with approve/reject — used by the baseline
 * prompt-revise flow to show the agent's minimal edit before it lands. LCS word
 * diff; additions green, deletions struck through in red. Spec §4.4.
 */
      ...
      <div className="max-h-64 overflow-y-auto px-3 py-2 text-sm font-mono leading-relaxed whitespace-pre-wrap">
        {parts.map((p, i) =>
          p.type === 'same' ? (
            ...
            <span key={i} className="bg-green-500/20 text-green-700 dark:text-green-300 rounded-sm">
              {p.text}
            </span>
          ) : (
            <span key={i} className="bg-red-500/20 text-red-700 dark:text-red-300 line-through rounded-sm">
              {p.text}
            </span>
          )
        )}
