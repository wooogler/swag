...
>
> **일부 폐기 (08-15 · 08-18).** 아래 `measure-store` 절의 `recordGuess`/`recordPointing`
> 2단 저장과 `guess_first` 게이트는 2-pass 개정(08-15)으로 사라졌다 — 지금은 세 응답이
> `recordPrediction` 한 번에 들어간다. Pass 1 순서도 **포인팅 → 서술 → 짐작**으로 바뀌었고
> (08-18), UI는 순서를 잠그지 않는다. 현행 기준은 `USER_STUDY 설계 v2.md` §2·§5와 코드다.
>
> **확정된 결정.**
