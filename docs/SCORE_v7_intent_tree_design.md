...
- **이미 살아있는 정의는 만들 수 없다.** 행이 아니라 **현재 정의 텍스트** 기준으로 막으므로, 손으로 똑같이 타이핑한 경우도 잡히고 기존 것을 열어보고 다르게 고치는 것도 가능하다. 목록에는 아무 표시도 하지 않는다 — 회색/취소선이 섞인 목록은 고장난 것처럼 읽힌다. 눌렀을 때 오른쪽 pane에서 이유를 말한다.
