...
**⑧ 후속 (2026-08-20) — 강제 문구의 조건을 코드로.** 데모에서 "예문 하나 달라"는 피드백이 빈칸 강제 룰을 낳았다(재현 9/9, 챗봇도 따름). 프롬프트 안에서 조건을 다는 방법 4종이 전부 실패해(오염 0/9은 되지만 억제 케이스가 4/6/2·6/6/0·7/4/1로 무너짐, 무조건은 11/1/0), `classifyWithholding`(mini·low·15s·실패시 false)이 피드백을 먼저 판정하고 프롬프트는 무조건 문장을 받거나 안 받는 구조로 바꿨다. **오염 0/9 + 억제 12/0/0**(역대 최고), 분류기 15/15. 프롬프트에 "챗봇의 장치를 없애는 것은 withholding이 아니다"를 명시해야 JELSON의 "do not give fill in the blanks"가 통과한다(그 전 14/15, 하필 빈칸을 재주입하는 케이스).
