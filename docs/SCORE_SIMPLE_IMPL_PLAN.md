...
> **⚠ 2026-08-21 개정.** 아래 본문은 08-20 시점 계획이라 **중첩 intent 트리를 전제한 서술이 남아 있다**(§5.1·§5.3 관련 문단, DDL 주석의 `parentSid`). 08-21에 중첩은 폐지되고 intent는 평면 순서 리스트가 되었으며, 생성 진입점이 쿼리 행으로 옮겨졌다 — 커밋 `4855a72`, 근거와 확정 사항은 `docs/SCORE_SIMPLE_DESIGN.md` **v2.2 §5.1 / §5.3 / §5.3a**. 충돌하면 설계 문서 v2.2가 우선이다.
