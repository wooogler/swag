...
## 9. 부록 — "깨끗한 규칙이 왜 안 먹히는가" 실험 기록 (2026-08-04)

문제 사례: "too whimsical" 앵커(message 1837, NIRVANA 마스터), 이전 18턴(그중 assistant 9턴 = 완성 산문 대필), 빈 Drafting root 규칙, 피드백 "쓰지 말고 표현을 보여줘". 장치 없는 2문장 금지 규칙("do not write the text… show expressions…")을 고정하고 레버별로 측정 (판정: LLM judge, compliant/partial/violation).

**규칙 문구 성분 (k=2, 실제 18턴, gpt-5.4-mini):**
| 성분 | 결과 |
|---|---|
| 선례 부인 절만 | 0/2 |
| 형식 계약(섹션·상한)만 | 1/2 |
| 형식 계약 + 부인 | **2/2** |
| + 오프닝 강제 | **2/2** (준수 생성은 전부 강제 헤딩으로 시작) |

**히스토리 개입 (k=2, 깨끗한 규칙 고정, gpt-5.4-mini):** 18턴 0/2 · 마지막 2턴 0/2 · 마지막 6턴 0/2 · **히스토리 제거 1/2** · 규칙 재진술(히스토리 뒤) 0/2 · 히스토리를 인용 컨텍스트로 포장 0/2 · "지시 갱신됨" 마커 0/2. → **히스토리는 증폭기(50%→100% 실패)일 뿐, 제거해도 반반. 히스토리 측 기계 개입으로는 구제 불가.**

**모델 티어 (k=3, 깨끗한 규칙 + 실제 18턴):** gpt-5.4-mini+reasoning medium = 3×partial(문단 소멸, 완성 문장 잔존) · gpt-5.4 = 3×violation · gpt-5.5 = 1 violation/2 partial. → **어떤 티어도 깨끗한 규칙을 준수까지 못 끌고 감. 모델 상향은 답이 아님.** (워크플로 C5의 "5.4 준수"는 합성 2턴 히스토리 한정 — 실제 스레드에서 미재현.)

**결론:** "장치 없는 깨끗한 규칙이 그냥 적용되는" UX는 현 세대 모델에서 달성 불가. 집행 장치(헤딩·상한·부인·오프닝)는 하중을 받는 구조물이며, 저작 프롬프트가 이를 자동 생성하는 현 설계가 유일하게 준수까지 도달한 레버. 사용자 소유감 문제는 장치 제거가 아니라 장치의 표현(라벨을 피드백 어휘에서 차용, 피드백 유형 비례, note 설명)으로 풀어야 함. 재측정 도구: `scripts/score/propose-eval.ts`.

**다이제스트 컨텍스트 실험 (2026-08-04 추가, k=3; 같은 날 preview-service v4로 채택 — 최종 파이프라인 전수 측정: 변형 3종 × 2샘플 × 2생성 = 12회 중 11 compliant / 1 partial / 0 violation):** 이전 대화를 turn 재생 대신 **요약 브리프**(챗봇 말투 배제, 참조 초안은 "학생의 현재 초안"으로 재귀속·verbatim 인용)로 가공해 단일 user 메시지로 제공:
| | 깨끗한 규칙 | 장치 규칙 |
|---|---|---|
| 실제 18턴 재생 (현행) | violation 전부 | ~75% 준수 |
| 다이제스트 | partial 전부 (**전문 대필 소멸**, 완성 문장 옵션 잔존; 초안 미인용 시 3/3 partial) | **3/3 준수** |

→ 다이제스트는 ① 최악 실패(통째 대필)를 깨끗한 규칙에서도 제거하고 ② 장치 규칙을 천장(100%)까지 올리며 ③ "배포는 학기 후 = 미래 대화는 새 규칙 아래 fresh 시작"이라는 실제 런타임을 오히려 더 잘 근사함(preview=runtime 불변식의 재해석: 과거 재생이 아니라 미래 런타임 근사). 다이제스트는 rule 무관이므로 (conversation, anchor) 단위 1회 생성·캐시 가능, preview 입력도 21k→~2k chars로 절감. 채택 시 PREVIEW_VERSION bump 필요. 초안 인용은 유지해야 응답 품질이 유지됨(비인용 시 일반론화) — 준수는 장치가 담당.

**2026-08-19 재측정 — 강제 3줄 → 1줄.** 사용자가 "그런 게 프롬프트에 있는 줄도 몰랐다"며 전면 삭제를 검토. 같은 시나리오·현행 파이프라인에서 4개 설정을 각 12생성 측정(compliant/partial/violation): **세 줄(기존) 8/4/0 · 미완성-예시 한 줄만 9/3/0 · 같은 취지를 추상적으로("붙여넣을 수 없게") 3/8/1 · 전부 제거 1/11/0.** 결론 = 효과는 **구체적 형태 요구 한 줄**이 전부다. 셀 수 있는 상한과 pushback 줄은 강제하지 않아도 모델이 알아서 쓰므로(제거 설정에서도 "at most 5", "briefly decline" 출현) 강제분은 순수 시스템 목소리였다. 추상화하면 효과가 사라지는 것은 type 분류기의 "무딘 지시가 이긴다" 패턴과 같다. JELSON 실제 앵커 3개(7059·7062·7064, 빈 rule 첫 수정)에서도 **한 줄 15/3/0 vs 세 줄 8/10/0**로 신설정이 우세. `buildProposeSystemPrompt(scope, {devices})` 기본값을 'form'으로 변경, 'all'/'goal'/'off'는 `propose-eval.ts --devices`로 재현용. 파일럿 분석의 "push back 절은 모델 습관"은 **오기** — 우리 프롬프트의 요구였다.

**2026-08-20 — 강제 문구의 조건을 프롬프트 밖으로.** 데모에서 *"Answer in one or two lines: give the word or spelling, then one short example sentence. No paragraph."*라는 피드백에 룰이 *"one short example sentence **as a stem with a blank or ellipsis, not a full paste-ready sentence**"*로 나왔다. 남은 강제 1줄이 원인 — "감춘 출력의 **대체물일 때**"라는 조건이 붙어 있는데 모델이 무조건 적용한다. 실제 intent 8350으로 재현: **9/9 룰이 빈칸을 강제**했고 챗봇도 그대로 따랐다(`exaggeration — The report may ___ the risks.`). 문구 변형 4종(조건 매몰/조건을 앞으로/조건+명시적 부정/인스트럭터 우선 선언) 전부 실패 — 오염은 0/9로 잡히지만 억제 케이스가 각각 4/6/**2**, 6/6/0, 7/4/**1**(무조건 11/1/0 대비). **조건을 코드로 뺐다**: `classifyWithholding`(gpt-5.4-mini, low, 15s, 실패 시 false)이 피드백 한 문장만 보고 "학생이 요청한 출력을 빼앗는가"를 판정하고, 프롬프트는 **무조건 문장을 받거나 아무것도 안 받는다**. 결과 **오염 0/9 + 억제 케이스 12/0/0**(역대 최고). 분류기 정확도 15/15(파일럿·데모 실제 피드백; "do not give fill in the blanks"처럼 **챗봇의 장치를 없애는 것**은 withholding이 아니라는 판별을 프롬프트에 명시해야 통과 — 그 전엔 14/15였고 하필 그 케이스가 빈칸을 재주입했다).

**2026-08-19 두 번째 변경 — 룰이 자기 정의를 다시 쓰지 않게.** 파일럿의 rule 4개가 전부 "When a student asks you to X…, but not Y"로 시작한 원인은 빈-rule 강도 사다리가 **트리거 폭**으로 등급을 매긴 것(minimal=정확히 그 요청만, moderate=rephrase까지). 조건은 라우터의 일이고, fold로 정의가 바뀌면 rule 속 사본이 상해서 모델이 "해당 없음"으로 읽는다(one-layer라 else가 없어 지시 0). 사다리를 **행동 서술 깊이**로 바꾸고 "이 프롬프트는 이미 매치된 요청에만 실행되니 적용 조건을 쓰지 말라"를 명시. **30생성/arm 측정: 준수 25/5/0 vs 구 24/6/0(동률), 조건절로 시작한 rule 12/18 → 0/18, 과제 고유 주제어를 담은 rule 2/18 → 0/18.**
첫 시도는 실패했다 — minimal을 "행동만"으로 두자 canonical에서 **2/2/2**(구 4/2/0). 금지만 있는 rule은 모델이 대체물을 스스로 고르고 완성 문장을 고른다. 구 사다리가 우연히 공급하던 "대신 무엇을"을 **모든 등급에** 넣어 해결. 재현: `--scoping trigger`.
앵커 인용 금지도 같은 라운드: 프롬프트 한 줄 + 라우트에서 앵커 n-gram(길이=앵커 단어수, 최대 5) 검출 시 해당 변형 제외(전부 걸리면 전부 통과 — 문체 규칙이 저작을 막지 않는다). 금지 전 39개 rule 중 1개가 인용("a machine doing better at"), 금지 후 39개 중 0개.

**다이제스트 채택 후 저작 프롬프트 경량화 (2026-08-04):** 무거운 장치(헤딩 명명·오프닝 강제·선례 부인)는 재생 시대의 반(反)전례 무기였음 — 다이제스트가 그 적을 제거했으므로 걷어내고 재측정: 준수율 동일(11/1/0), 규칙 길이 절반(445–579자, 이전 600–1150자), 헤딩·워드버짓·부인 보일러플레이트 소멸 → 규칙이 강사 자신의 지시처럼 읽힘. 유지된 최소 요구: 금지엔 대신-무엇 명시 + 셀 수 있는 상한 1개, 대체 예시는 미완성(빈칸), pushback 한 줄, "짧게 — 폼 명세가 아니라 지시처럼".
