...
1. **응답 부분 선택 피드백**: 응답 텍스트 드래그 → 피드백 박스에 인용 삽입(“이 부분: …”) — LLM 그라운딩 정밀화. UI 소폭.
2. **intent별 누적 원칙(standing constraints)**: 확정된 changeIntents/피드백 원칙을 누적 저장해 매 propose에 상수 제약으로 주입 — 나중 제안이 이전 피드백을 되돌리는 회귀 방지. (definition의 pins→fold 패턴의 rule판. 스키마 추가 필요, 효과 검증 후.)
3. **Judge 재활용 검증**: 변형 적용 후 “이전 피드백 N개가 여전히 관철되는가”를 저비용 판정으로 뱃지 표시.
